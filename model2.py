...
# 参数
batch_size = 64  # 一次处理多少文本序列
block_size = 256  # 序列的长度（上下文窗口大小）
device = 'cuda' if torch.cuda.is_available() else 'cpu'
eval_iters = 100
n_embd = 384 # 嵌入维度
head_size = 64 # 注意力头的维度
n_head = 6 # 注意力头的数量
n_layer = 6 # Transformer 块的数量
...
class Block(torch.nn.Module):
    ...
    def forward(self, x):
        x = self.sa(self.ln1(x)) + x  # 注意力层的输出加上输入（残差连接）
        x = self.ffwd(self.ln2(x)) + x # 前馈网络的输出加上输入（残差连接）
        return x
...
def estimate_loss():
    ...
    for split in ['train', 'val']:
        # 在 device 上预分配张量以避免不必要的同步和复制
        losses = torch.zeros(eval_iters, device=device)
        for k in range(eval_iters):
            X, Y = get_batch(split)
            _, loss = model(X, Y)
            losses[k] = loss  # 不要在这里调用 .item() 否则会打断 GPU 执行流
        out[split] = losses.mean().item() # 最后统一汇回 CPU
    model.train()
    return out
...
for steps in range(4000):
    ...
    if steps % 1000 == 0:
        ...
        print(f"step {steps}: train loss {losses['train']:.4f}, val loss {losses['val']:.4f} (time: {time.time()-t0:.2f}s)")
        t0 = time.time()
